...
double* SortMerge(double *myVector, double *myBuffer, 
	              unsigned borderLeft, unsigned borderRight)
{
	...
	// divide and rule
	double *bufferLeft = 
		SortMerge(myVector, myBuffer, borderLeft, borderMiddle);
	double *bufferRight = 
...
int main(int argc, char *argv[])
{
	...
	if (myId == 0) {
		...
		if (vectorSize <= 10)
			GetVectorInfo(lnVector, vectorSize);
		...
		std::cout << std::endl << "ShellSortTime: " 
			      << lnWorkTime << std::endl;
	...
	MPI_Scatter(plVector, blockSize, MPI_DOUBLE, 
		        plBlock, blockSize, MPI_DOUBLE, 0, MPI_COMM_WORLD);

	SortShell(plBlock, blockSize);

	MPI_Barrier(MPI_COMM_WORLD);

	MPI_Gather(plBlock, blockSize, MPI_DOUBLE, 
		       plVector, blockSize, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	...
	if (myId == 0) {
		...
		std::cout << std::endl << "ShellMergeSortTime: " 
			<< plWorkTime << std::endl;

		if (vectorSize <= 10)
			GetVectorInfo(lnVector, vectorSize);

		std::cout << std::endl << "SPEED-UP: " 
			<< lnWorkTime / plWorkTime << std::endl;
	}
